...
interface IOCDataMap {
  [OCid: string]: IEncryptedData;
}

interface IEncryptedMap {
...
export class Umbral {
  ...
  public encryptData(randIds: Uint8Array[], record: IRecord, pkOCs: IKey[],
    ...
    for (const randId of randIds) {
      // each matching index
      const OCDict = this.createEncryptedObject(randId, record, pkOCs, userPassPhrase);
      console.log('dict', OCDict);
    }
  ...
  private createEncryptedObject(randId: Uint8Array,
                                record: IRecord, pkOCs: IKey[],
                                userPassPhrase: Uint8Array): IOCDataMap {
    const OCDict: IOCDataMap = {};
    try {
      ...
      for (const pkOC of pkOCs) {
          const eOC = this.asymmetricEncrypt(JSON.stringify(msg), pkOC.key);
          OCDict[pkOC.id] = {id: recordId, matchingIndex: derived.matchingIndex, eOC, eRecord, eUser};
      }
      console.log(OCDict);
      return OCDict;
    } catch (e) {
